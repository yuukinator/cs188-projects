...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    bestLocation = None
    action = None
    numLivingGhosts = len(livingGhosts)
    for i in range(numLivingGhosts):
    	if livingGhosts[i] == 1: 
    		beliefsIndex = i - 1
    		bestProbDist = max(self.ghostBeliefs[beliefsIndex].values())
    		for g in self.ghostBeliefs[beliefsIndex]:
    			if self.ghostBeliefs[beliefsIndex][g] == bestProbDist:
    				tempLocation = g
    		mazeDistance = self.distancer.getDistance(tempLocation, pacmanPosition)
    		if mazeDistance < closestDist:
    			closestDist = mazeDistance
    			bestLocation = tempLocation
    		
    legalActions = legal
    for act in legalActions:
    	successorPosition = Actions.getSuccessor(pacmanPosition, act)
    	if self.distancer.getDistance(bestLocation, successorPosition) < closestDist:
    		action = act
    return action