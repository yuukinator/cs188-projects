...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    """
    "*** YOUR CODE HERE ***"
    possPositions = self.legalPositions
    numParticles = self.numParticles
    self.particleList = []
    for i in range(numParticles):
    	self.particleList.append(random.choice(possPositions))
  ...
  def observe(self, observation, gameState):
    ...
    for i in self.particleList:
    	trueDistance = util.manhattanDistance(pacmanPosition, i)
    	weight = emissionModel[trueDistance]
    	particleCounter[i] += weight
    particleCounter.normalize()
    	
    if noisyDistance == None:
    	for p in self.particleList:
    		if p == self.getJailPosition():
    			particleCounter[p] = 1.0
    		else:
    			particleCounter[p] = 0.0
    
    resampleParticles = util.Counter()
    weightSum = particleCounter.totalCount()
    if weightSum == 0:
		self.initializeUniformly(gameState)
    if weightSum != 0:
    	tempList = []
    	for i in range(self.numParticles):
    		tempList.append(util.sample(particleCounter))
  ...
  def elapseTime(self, gameState):
    ...
    for i in self.particleList:
    	oldPos = i
    	newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
    	tempParticleList.append(util.sample(newPosDist))
	self.particleList = tempParticleList
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    self.particles = []
    for i in range(self.numParticles):
    	self.particles.append(tuple([random.choice(self.legalPositions) for ghosts in range(self.numGhosts)]))
    ...
    ** Remember ** We store particles as tuples, but to edit a specific particle,
    ...
    for p in self.particles:
    	...
    	for g in range(self.numGhosts):
    		trueDistance = util.manhattanDistance(particleList[g], pacmanPosition)
    		if noisyDistances[g] == None:
    			p = self.getParticleWithGhostInJail(p, g)
    			emission = 1
    			particleWeight *= emission
    		else:
    			emission = emissionModels[g]
    			particleWeight *= emission[trueDistance]
    	weightedParticles[p] += particleWeight
    	...
    	for g in range(self.numGhosts):
    			if noisyDistances[g] == None:
    				for p in range(self.numParticles):
    					tempList[p] = self.getParticleWithGhostInJail(self.particles[p], g)
    	self.particles = tempList

    else:
    	for i in range(self.numParticles):
    		tempList2.append(util.sampleFromCounter(weightedParticles))
